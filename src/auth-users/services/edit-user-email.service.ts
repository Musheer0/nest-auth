...
export const EditUserEmail = async (
  ...
  if (!metadata.user_agent || !metadata.ip) throw new BadRequestException("Invalid request");
 ...
 // Check if email still free
  ...
  if (token && code) {
    const vToken = await prisma.verification_token.findFirst({
      where: { id: token, user_id: user.id, scope: VerificationTokenScope.EDIT },
    });

    if (!vToken) throw new BadRequestException("Invalid token");
    if (vToken.expires_at <= new Date()) throw new BadRequestException("Token expired");
