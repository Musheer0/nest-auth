...
import {
  fifteenDaysFromNow,
  fifteenMinsFromNow,
} from 'src/utils/others/date-utils';
...
export const CrendentialsSignIn = async (
  prisma: PrismaClient,
  ...
  await SendEmail({
    to: user.email!,
    html: generateOtpEmail(user.email!, otp_token.token, 'Your MFA token'),
    title: 'Multifaction Authentication',
  });
