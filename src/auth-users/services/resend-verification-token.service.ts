...
export const ResendEmailVerificationToken= async(prisma:PrismaClient,metadata:UserMetaData,token?:string,user_id?:string,email?:string)=>{
   ...
   if(old_token && old_token.created_at > new Date(Date.now() - 5 * 60 * 1000)){
    throw new BadRequestException("please wait 5 mins before requesting again");
    }
  ...
  await SendEmail({to:user.email,html:generateOtpEmail(user.email,otp_token.token),title:'Verify your email'},)
  return {
    token: new_token.id,
